...
async def test_step_draft_returns_text():
    mock_response = _mock_message("# Generated Document\n\nContent body.")
...
async def test_step_eval_parses_markdown_codeblock():
    ...
    mock_response = _mock_message(wrapped)
    with patch("pipeline.client.messages.create", new=AsyncMock(return_value=mock_response)):
...
async def test_run_pipeline_emits_complete_event():
    draft_response = _mock_message("# Draft content")
    eval_data = {"overall": 8, "clarity": 7, "structure": 9, "completeness": 8}
    eval_response = _mock_message(json.dumps(eval_data))

    with patch(
        "pipeline.client.messages.create",
        new=AsyncMock(side_effect=[draft_response, eval_response]),
    ):
...
async def test_run_pipeline_step_sequence():
    ...
    # Steps 1-4: each has running + done = 8 step events
